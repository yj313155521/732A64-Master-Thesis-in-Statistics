...
### The following function is adapted to suit the task2 and this is different from that in version_2_code_thesis
RMSE_func <- function(matrix_1, matrix_2, num_factors){
  
  seq_vec <- c() # This is used to illustrate the real sequence after the reorder process.
  
  meta_vector <- c(1:num_factors) # Ensure only the first "num_factors" columns in the loading matrix gotten from the EFA should be considered.
...
## Define the real number of factors behind observable variables.
num_factors <- 3
...
for(k in 5:10){
  ...
  for(l in 1:sim_num){
    ...
    RMSE <- RMSE_func(fl,loadings_df,num_factors)
    ...
    ## Calculate the RMSE(rooted mean square error)
    RMSE_2 <- RMSE_func(fl,loadings_df_2,num_factors)
    ...
    ## Calculate the RMSE(rooted mean square error)
    RMSE_3 <- RMSE_func(fl,loadings_df_3,num_factors)
