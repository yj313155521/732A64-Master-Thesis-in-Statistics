...
# Comparsion between Different Symmetry Conditions with 5 Factors (Simulation and EFA)
data_1 <- read.csv("Experiment_Result/1.3/RMSE_result_1.csv")
data_2 <- read.csv("Experiment_Result/2.3/RMSE_result_1.csv")
data_3 <- read.csv("Experiment_Result/3.3/RMSE_result_1.csv")
...
Main_Plot <- ggplot() +
  ...
  geom_errorbar(aes(x = `Number of Categories`, y = RMSE, ymin = RMSE - 1.96*sd/sqrt(60), ymax = RMSE + 1.96*sd/sqrt(60), color = `Symmetry Condition`), 
